...
- Accounts, an all-time career leaderboard, and more off-field variety (2026-09-23, user request, "we also need a real leaderboard that gets tracked automatically and you have a login for the site if you want to play 5 star or hot seat" plus "more variation in selections that pop up in season like coach hires, or players you pick up or answers you give"):
  - **Accounts:** login/signup on `@netlify/identity` — thin server-side wrappers (`netlify/functions/auth-*.js`) around the package's own `signup()`/`login()`/`logout()`/`getUser()`, each mutation-performing one carrying the `verifyRequestOrigin()` CSRF check the package's README calls for. Chose this over the classic `netlify-identity-widget.js` because Netlify's current docs recommend `@netlify/identity` for new projects and the widget's client-managed-JWT model doesn't line up cleanly with the newer cookie-session `getUser()` API. Login/signup end in a full page reload rather than patching state in place — the package's own README documents that as the reliable way to guarantee the browser has the new session cookie before the next read, not a stylistic choice.
  - **Gate:** `showLegacyIntro()` (`career_d.js`), the single choke point both Five-Star and Hot Seat pass through before the character builder, calls `requireLogin(mode)` first. That function only enforces anything once a real round trip to `/api/auth-user` has succeeded — on a host without the auth functions (the Claude Artifact, or Netlify before Identity is turned on) it resolves true immediately and nothing changes. [VERIFIED against the local dev server, which has none of these functions: the account button in the header stays hidden and Five-Star opens straight to the character builder, same as before this round — see the earlier version of this bug, caught by that exact check: `backendHasAuth()` first read "did we finish a check" instead of "did the check succeed," which would have shown the login wall on the Artifact too; fixed before this was committed.]
  - **All-time career leaderboard:** `netlify/functions/submit-career.js` / `career-leaderboard.js`, Netlify Blobs again, one list per mode. A finished career (`finishPlayerCareer`/`finishCoachDynasty`, `career_d.js`) posts automatically under the signed-in account — no name entry, unlike the daily board, since the account already has one (`netlify/functions/profile.js`, asked for once). Ranked by `careerScore()` (`logic_d.js`, pure, tested): for Five-Star, wins/losses, Heisman, All-American, titles and draft slot; for Hot Seat, wins/losses, titles, CFP trips, Hall of Fame, and a penalty for getting fired. Each account keeps only its single best finish per mode. [MEASURED via `tests/balance.html`: a Heisman-winning title season scores 388 vs. 42 for a plain winning one; a Hall of Fame dynasty scores 650 vs. -42 for a fired one — the formula clearly separates a great career from an ordinary one, which is the property that matters, not the exact point values.] Same honor-system, no-replay-validation posture as the daily board, named for the same reason.
  - **Not verified:** the actual login/signup/leaderboard network round trip against a real deployment — same limit as the daily leaderboard when it was built, for the same reason (the local dev server has no `/api/*` routes at all). Worth a live pass, the same way the daily leaderboard's submit/read cycle was checked by hand after deploying.
  - **More variation, text-only:** recruiting (`coachOffseason`, `career_d.js`) drew from one fixed script every single year — same 3 choice labels, same 3 outcome sentences, forever. Now draws a featured recruit from a 7-position pool and 2-3 outcome-text variants per branch; the offensive-coordinator hire (previously the only staff scene, 30% chance a year) is now a 4-scenario pool. The off-field `EVENTS` pool (`career_c.js`) grew from 18 entries to 33 (9 new player, 7 new coach) — over a 5-8 year career the old pool visibly cycled (`C.recentEv` only blocks the last 3 repeats), this nearly doubles how much is seen before that happens. Deliberately not touched: any pricing/odds math (`priceScene`, `landProb`) or the recruiting rating formula — this pass is which words and which choices show up, not how much they're worth. [VERIFIED: all 15 new events build without error and price correctly through the existing `priceScene` pipeline — checked directly via the console rather than waiting to encounter each one by chance in play; full balance suite stayed at 43/43, and the existing risky-answer pricing check picked up 8 more options to verify (24 → 32) without needing a new test written for it.]
